...
class ProposalTargetCreator(object):
    ...
    def __call__(self, roi, bbox, label, loc_normalize_std=(0.1, 0.1, 0.2, 0.2)):
        ...
        # construct IOU weights
        pos_IOU = max_iou[pos_index]
        neg_IOU = max_iou[neg_index]
        alpha = 1.75
        gamma = 1.1
        pos_weights = np.power(1 - pos_IOU, 0) # alpha * (1 - np.power(1 - pos_IOU, gamma))
        neg_weights = alpha * np.power(1 - neg_IOU, gamma)
        IOU_weights = torch.tensor(np.append(pos_weights, neg_weights)).unsqueeze(1).cuda()
        neg_IOUs = torch.tensor(neg_IOU.mean()).cuda()
        # print(IOU_weights.shape)

        sample_roi = roi[keep_index]
        if len(bbox)==0:
            return sample_roi, np.zeros_like(sample_roi), gt_roi_label[keep_index], IOU_weights, neg_IOUs
        ...
        gt_roi_label = gt_roi_label[keep_index]
        gt_roi_label[pos_roi_per_this_image:] = 0
        return sample_roi, gt_roi_loc, gt_roi_label, IOU_weights, neg_IOUs
...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        for i in range(n):
            ...
            #   gt_roi_label    [n_sample, ]
            # ------------------------------------------------------ #
            sample_roi, gt_roi_loc, gt_roi_label, IOU_weights, neg_IOU = self.proposal_target_creator(roi, bbox, label, self.loc_normalize_std)
            ...
            roi_cls_loc, roi_score, ht_score = self.faster_rcnn.head(torch.unsqueeze(feature, 0), sample_roi, sample_roi_index, img_size)
            ...
            all_cls_loss = (self.cross_entropy(roi_score[0], gt_roi_label) * IOU_weights).mean()
            
            # compute rare classes loss
            ht_roi_label = torch.zeros_like(gt_roi_label).cuda()
            ht_roi_label[gt_roi_label > 2] = 1 
            # 1-car; 2-taxi; 3-bus; 4-minibus
            # ht_cls_loss = nn.CrossEntropyLoss()(ht_score[0], ht_roi_label)
            ht_cls_loss = (self.cross_entropy(ht_score[0], ht_roi_label) * IOU_weights).mean()
            
            roi_cls_loss = all_cls_loss + ht_cls_loss * 3
            roi_loc_loss  = roi_loc_loss + neg_IOU * 5
